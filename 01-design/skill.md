...
**Colors 配对体系（对齐 Material Design，确保文字与背景对比度）**：

每个主要颜色**必须**配对定义 `on-*` 文字色：
- `primary` / `on-primary`：主色 + 主色上的文字色
- `primary-container` / `on-primary-container`：主色容器 + 容器内文字
- `secondary` / `on-secondary`：次要色 + 次要色上的文字
- `secondary-container` / `on-secondary-container`：次要色容器 + 容器内文字
- `surface` / `on-surface`：页面背景 + 背景上的正文
- `surface-container` / `on-surface-container`：卡片背景 + 卡片内文字
- `error` / `on-error`：错误色 + 错误背景上的文字
- `error-container` / `on-error-container`：错误容器 + 容器内文字

**为什么必须配对**：AI 实现时直接用 `backgroundColor: {colors.primary}` + `textColor: {colors.on-primary}`，永远不会出现白字压白底的对比度问题。
